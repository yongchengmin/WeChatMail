...
public class TemplateMsg {
	public static String sendTemplateMsg(String requestUrl,String token,String vin,String status,String remark
			,String openid,String detailUrl){
		//用户在该公众号下的openid
		//3 调用模板消息的发送方法
		String templateId = PropertiesUtil.getPropertiesKey(WeChartGlobal.PORTMESG, WeChartGlobal.TEMPLATE_ID);
		Template tem=new Template();
		tem.setTemplateId(templateId);//为模板id 
		tem.setTopColor(WeChartGlobal.topColor);
		tem.setToUser(openid);
		tem.setUrl(detailUrl);//根据不同的底盘号,传递不同的URL,前端人员完成
